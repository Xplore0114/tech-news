...
const MARKET = {{
  ...
  cn: [
    {{s:"BABA",n:"阿里巴巴"}},{{s:"0700.HK",n:"腾讯"}},{{s:"JD",n:"京东"}},
    {{s:"BIDU",n:"百度"}},{{s:"PDD",n:"拼多多"}},{{s:"BILI",n:"哔哩哔哩"}},
  ],
  metal: [
    {{s:"GC=F",n:"黄金",unit:"oz"}},
    {{s:"SI=F",n:"白银",unit:"oz"}},
...
function mCard(name, sym, extra, q) {{
  ...
  return `<div class="mcard ${{cls}}">
    ...
    <div class="mc-price">${{q.currency}} ${{q.price.toFixed(2)}}</div>
...
async function loadMarket(force) {{
  ...
  document.getElementById('mg-us').innerHTML    = MARKET.us.map(x    => mCard(x.n, x.s, '', map[x.s])).join('');
  document.getElementById('mg-cn').innerHTML    = MARKET.cn.map(x    => mCard(x.n, x.s, '', map[x.s])).join('');
  document.getElementById('mg-metal').innerHTML = MARKET.metal.map(x => mCard('🥇 '+x.n, x.s, ' / '+x.unit, map[x.s])).join('');
...
async function loadOverviewMarket() {{
  ...
  document.getElementById('ov-market-grid').innerHTML = picks.map(x => mCard(x.n, x.s, '', map[x.s])).join('');
  ovMarketLoaded = true;
}}
